...
opts.show          =   true;
tr_dat = trainImgs';
trls = cell2mat(values(imgStateHashTable, trainLabels));
tt_dat = testImgs';
ttls = cell2mat(values(imgStateHashTable, testLabels));
...
precision=0;
for i=1:6
    precision = precision+sum(ID(find(ttls==i)) == ttls(find(ttls==i))')/numel(find(ttls==i));
end
